...
export default function OperatorCopilotPage() {
  const user = DEMO_USERS.find((u) => u.role === "operator")!;
  const [messages, setMessages] = useState(COPILOT_MESSAGES.operator);
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      const aiResponses = [
        "I found the cheapest compliant route for you. Let me share the details.",
        "Your permit status is currently pending review. It should be approved within 24 hours.",
        "Here are your billing details for this month's corridor usage.",
      ];
  ...
  return (
    <AppShell role="operator" user={user}>
        ...
        <div className="card" style={{ flex: 1, display: "flex", flexDirection: "column", marginBottom: "24px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "16px", paddingBottom: "16px", borderBottom: "1px solid var(--border)" }}>
            ...
            <div>
              ...
              <p style={{ fontSize: "12px", color: "var(--muted)", margin: 0 }}>Ask about routes, permits, or billing</p>
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: "12px", alignItems: "center", borderTop: "1px solid var(--border)", paddingTop: "16px" }}>
            <input
              ...
              placeholder="Ask something like 'Show me cheapest route'..."
              style={{
                flex: 1,
                padding: "12px 16px",
                fontSize: "14px",
