...
export default function MunicipalityActivityPage() {
  ...
  const filtered = useMemo(() => {
    let list = activityEntries.filter((e) => 
      e.userRole === "municipality" || e.userRole === "operator"
    );
    ...
    <AppShell role="municipality" user={municipalUser}>
      <div style={{ marginBottom: "24px" }}>
        ...
        <p style={{ color: "var(--muted)", fontSize: "14px", marginBottom: "16px" }}>
          Municipal actions and operator activity in your area
        </p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "12px" }}>
          ...
          <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "14px" }}>
            <div style={{ fontSize: "11px", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: "4px" }}>Flights Completed</div>
            <div style={{ fontSize: "22px", fontWeight: "800", color: "var(--accent)" }}>{filtered.filter(e => e.type === "completed").length}</div>
          </div>
          <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "10px", padding: "14px" }}>
              ...
              <div key={e.id} style={{ display: "flex", gap: "14px", padding: "14px 18px", borderBottom: "1px solid var(--border)", alignItems: "flex-start" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "11px", color: "var(--muted)", marginTop: "4px" }}>
                    {e.userName} · {e.userRole} · {formatTime(e.timestamp)}
                  </div>
                </div>
              </div>
            );
